...
class SplitController {
  ...
  public:
	...
	uint32_t getSlaveButtonState() {
		...
		uint8_t regwrite[1] = { 0x00 };  // change the address to 0x00

		int result = i2c_write_blocking(i2c0, 0x17, regwrite, 1, false);

		uint32_t slaveButtonState = 0xFFFFFFFF;

		if (result >= 0) {  // if the write operation is successful

			uint8_t idRead[4] = { 0 };
			result = i2c_read_blocking(i2c0, 0x17, idRead, 4, false);

			// Check if result is equal to 4
			if (result == 4) {
				slaveButtonState = idRead[0] | (idRead[1] << 8) | (idRead[2] << 16) | (idRead[3] << 24);
			}
		}
